  ...
  const handleVote = async () => {
    ...
    if (!contractAddress || contractAddress === "0x0000000000000000000000000000000000000000") {
      error(
        ...
        `The qualification contract is not available yet. Voting opens April 14 at 10am CT.`
      )
      return
    }
          ...
          {isPreLaunch && (
            <div className="bg-accent/10 border border-accent/30 rounded-sm p-4 text-center space-y-2">
              <div className="flex items-center justify-center gap-2">
                ...
                <span className="text-sm font-bold text-accent uppercase tracking-wide">Voting opens April 14 at 10am CT</span>
              </div>
              <PreLaunchCountdown />
            </div>
          )}
